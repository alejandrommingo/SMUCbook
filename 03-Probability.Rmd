...
A set is a [primitive notion](https://en.wikipedia.org/wiki/Primitive_notion). That is, it is a basic idea of human thought. A set is a collection of objects: numbers, images ... anything, football players, words, colors ... 
...
### Basic characteristics and properties of the sets

* Each $x$ object of $\Omega$ is called **the element of the set** $\Omega$ and we say that $x$ belongs to $\Omega$.  We will denote it with $x\in \Omega$.
* A **set of one element**, for example, $\{1\}$ is called a **singleton**.
* Let $A$ be another set we will say that $A$ **is equal to** $B$ if all the $A$ elements are in $B$ and all the $B$ elements are in $A$. For example $A=\{1,2,3\}$ is equal to $B=\{3,1,2\}$.
...
We will assign a real number $\mathbb{P}(A)$ to every event $A$, called the *probability* of $A$. We also call $\mathbb{P}$ a *probability distribution* or a *probability measure*. 
